...
static const struct scanners_supported scanners[] = {
	...
	{ 
		...
		resolutions_list_300, resolutions_rounds_300, /* TO FIX */
		image_emphasis_list_3, image_emphasis_val_3,
		MAT_CAP_DUPLEX | MAT_CAP_CONTRAST | MAT_CAP_PAPER_DETECT | MAT_CAP_MIRROR_IMAGE
	},
	...
	{ 
		...
		image_emphasis_list_5, image_emphasis_val_5,
		MAT_CAP_DUPLEX | MAT_CAP_CONTRAST | MAT_CAP_AUTOMATIC_THRESHOLD	| 
		MAT_CAP_WHITE_LEVEL | MAT_CAP_GAMMA | MAT_CAP_NOISE_REDUCTION | MAT_CAP_PAPER_DETECT | 
		MAT_CAP_DETECT_DOUBLE_FEED | MAT_CAP_MANUAL_FEED	
	}
};
 ...
 * document size. */
static SANE_Status matsushita_read_document_size (Matsushita_Scanner *dev)
{
	CDB cdb;
	SANE_Status status;
	size_t size;
	...
	if (status != SANE_STATUS_GOOD || size != 0x10) {
		DBG (DBG_error, "matsushita_read_document_size: cannot read document size\n");
...
static int matsushita_build_paper_sizes (Matsushita_Scanner * dev)
{
	...
	for (i = 0, num = 0; i<NELEMS(paper_sizes); i++) {
		if (SANE_UNFIX(scanners[dev->scnum].x_range.max) >= paper_sizes[i].width &&
			SANE_UNFIX(scanners[dev->scnum].y_range.max) >= paper_sizes[i].length) {
...
static int get_string_list_index(SANE_String_Const list[], SANE_String_Const name)
{
	int index;
...
static int get_int_list_index(const SANE_Word list[], const SANE_Word value)
{
	int index;
...
matsushita_sense_handler (int scsi_fd, unsigned char *result, void *arg)
{
	...
	if ( get_RS_error_code(result) != 0x70 ) {
		DBG(DBG_error, "matsushita_sense_handler: invalid sense key error code (%d)\n", 
	...
	if (len < 14) {
		DBG(DBG_error, "matsushita_sense_handler: sense too short, no ASC/ASCQ\n");
	...
	DBG(DBG_sense, "matsushita_sense_handler: unknown error condition. Please report it to the backend maintainer\n");
...
matsushita_init_options (Matsushita_Scanner * dev)
{
	...
	dev->opt[OPT_MODE].size = max_string_size(scanners[dev->scnum].scan_mode_list);
	dev->opt[OPT_MODE].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_MODE].constraint.string_list = scanners[dev->scnum].scan_mode_list;
	...
	dev->opt[OPT_RESOLUTION].constraint.word_list = scanners[dev->scnum].resolutions_list;
	...
	dev->opt[OPT_BRIGHTNESS].constraint.range = &(scanners[dev->scnum].brightness_range);
	...
	dev->opt[OPT_CONTRAST].constraint.range = &(scanners[dev->scnum].contrast_range);
	...
	dev->opt[OPT_AUTOMATIC_THRESHOLD].desc  = SANE_I18N("Automatically sets brightness, contrast, white level, gamma, noise reduction and image emphasis");
	dev->opt[OPT_AUTOMATIC_THRESHOLD].type  = SANE_TYPE_STRING;
	dev->opt[OPT_AUTOMATIC_THRESHOLD].size = max_string_size(automatic_threshold_list);
	dev->opt[OPT_AUTOMATIC_THRESHOLD].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_AUTOMATIC_THRESHOLD].constraint.string_list = automatic_threshold_list;
	...
	dev->opt[OPT_HALFTONE_PATTERN].size = max_string_size(halftone_pattern_list);
	dev->opt[OPT_HALFTONE_PATTERN].cap |= SANE_CAP_INACTIVE;
	dev->opt[OPT_HALFTONE_PATTERN].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_HALFTONE_PATTERN].constraint.string_list = halftone_pattern_list;
	...
	dev->opt[OPT_NOISE_REDUCTION].desc = SANE_I18N("Reduce the isolated dot noise");
	...
	dev->opt[OPT_IMAGE_EMPHASIS].size = max_string_size(scanners[dev->scnum].image_emphasis_list);
	dev->opt[OPT_IMAGE_EMPHASIS].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_IMAGE_EMPHASIS].constraint.string_list = scanners[dev->scnum].image_emphasis_list;
	...
	sane_control_option (dev, OPT_MODE, SANE_ACTION_SET_VALUE,
						 (SANE_String_Const *) scanners[dev->scnum].scan_mode_list[0],
						 NULL);
}
...
matsushita_set_window (Matsushita_Scanner * dev, int side)
{
	...
	/* Image emphasis */
	i = get_string_list_index(scanners[dev->scnum].image_emphasis_list, dev->val[OPT_IMAGE_EMPHASIS].s);
	...
	if (dev->scan_mode == MATSUSHITA_HALFTONE) {
		i = get_string_list_index(halftone_pattern_list, dev->val[OPT_HALFTONE_PATTERN].s);
	...
	if (dev->scan_mode == MATSUSHITA_BW ||
		...
		/* Noise reduction */
		i = get_string_list_index(noise_reduction_list, dev->val[OPT_NOISE_REDUCTION].s);
		...
		i = get_string_list_index(automatic_threshold_list, dev->val[OPT_AUTOMATIC_THRESHOLD].s);
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_BR_X:
			...
			if (value & (scanners[dev->scnum].resolutions_round[i]-1)) {
				value = (value | (scanners[dev->scnum].resolutions_round[i]-1)) + 1;
				if (info) {
		...
		case OPT_PAPER_SIZE:
			...
			i = get_string_list_index(dev->paper_sizes_list, dev->val[OPT_PAPER_SIZE].s);
...
sane_get_parameters (SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!(dev->scanning)) {
		...
		dev->params.pixels_per_line = (((dev->width * dev->resolution) / 1200) + 7) & ~0x7;
		...
		dev->params.bytes_per_line = (dev->params.pixels_per_line / 8) * dev->params.depth;
		   ...
		   SANE_Int * len)
{
	...
	*len = 0;
	...
	if (dev->depth == 1) {
		...
		for (i=0; i<size; i++) {
			c = 0;
			if (buf[i] & 0x01) c |= 0x80; 
			if (buf[i] & 0x02) c |= 0x40; 
			if (buf[i] & 0x04) c |= 0x20; 
			if (buf[i] & 0x08) c |= 0x10; 
			if (buf[i] & 0x10) c |= 0x08; 
			if (buf[i] & 0x20) c |= 0x04; 
			if (buf[i] & 0x40) c |= 0x02; 
			if (buf[i] & 0x80) c |= 0x01; 
			buf[i] = c;
		}
	}
	else if (dev->depth == 4) {
