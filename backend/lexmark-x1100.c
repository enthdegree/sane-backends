...
static SANE_Status x1100_write_all_regs (SANE_Int devnum, SANE_Byte * regs);
...
static SANE_Status x1100_get_start_loc (SANE_Int resolution,
					...
					SANE_Int offset,
					Lexmark_Device * dev);
static void x1100_rewind (Lexmark_Device * dev, SANE_Byte * regs);
...
static SANE_Status x1100_simple_scan (Lexmark_Device * dev,
				      SANE_Byte * regs,
				      int xoffset,
				      int pixels,
				      int yoffset,
		    ...
		    SANE_Int length)
{
  ...
  if ((start + length > 0xb3) && (length > 1))
    {
      size = 0xb3 - start;
      if (x1100_usb_bulk_write (devnum, source, &size) != SANE_STATUS_GOOD)
	{
		   ...
		   size_t * size)
{
  ...
  cmd_size = 4;
  status = x1100_usb_bulk_write (devnum, read_cmd, &cmd_size);
  if (status != SANE_STATUS_GOOD)
    {
      *size = 0;
  ...
  status = x1100_usb_bulk_read (devnum, data, size);
  if (status != SANE_STATUS_GOOD)
    {
      *size = 0;
...
rts88xx_commit (SANE_Int devnum, SANE_Byte depth)
{
  ...
  status = x1100_stop_mvmt (devnum);

  /* effective start */
  status = x1100_start_mvmt (devnum);
 ...
 */
static SANE_Status
lexmark_x1100_set_idle (SANE_Int devnum)
{
  SANE_Byte regs[14] =
  ...
  if (rts88xx_write_regs (devnum, 16, regs, 14) != SANE_STATUS_GOOD)
    {
      DBG (5, "lexmark_x1100_set_idle : register write failed ...\n");
      return SANE_STATUS_IO_ERROR;
    }
  return SANE_STATUS_GOOD;
}
...
/* wake up scanner */
static SANE_Status
lexmark_x1100_wake_up (Lexmark_Device * dev)
{
  ...
  for (i = 0; i < 10; i++)
	...
	{
	  DBG (5,
	       "lexmark_x1100_wake_up : register write pass %d failed ...\n",
	       i);
	  return SANE_STATUS_IO_ERROR;
	}
    }
 ...
 */
SANE_Status
sanei_lexmark_x1100_init (Lexmark_Device * dev)
{
  int i;
  SANE_Status status;

  status = SANE_STATUS_UNSUPPORTED;
  DBG (2, "x1100_init: start\n");
  DBG (5, "x1100_init: code version rc1");
  ...
  DBG (5, "sanei_lexmark_x1100_init: init done for model %s/%s\n",
       dev->model.model, dev->model.name);
  DBG (2, "x1100_init: done\n");
  return status;
}
...
x1100_usb_bulk_read (SANE_Int devnum, SANE_Byte * buf, size_t * size)
{
  ...
  DBG (7, "x1100_usb_bulk_read: returned size = %ld (required %d)\n",
...
/* stops current scan */
static SANE_Status
x1100_cancel (SANE_Int devnum)
{
  SANE_Status status;

  DBG (2, "x1100_cancel: start\n");
  status = x1100_stop_mvmt (devnum);
  if (status != SANE_STATUS_GOOD)
    return status;
  status = x1100_clr_c6 (devnum);
  if (status != SANE_STATUS_GOOD)
    return status;
  DBG (2, "x1100_cancel: end.\n");
  return status;
}

static SANE_Status
x1100_start_scan (SANE_Int devnum, SANE_Byte * regs)
{
  SANE_Status status;

  DBG (2, "x1100_start_scan: start\n");

  /* writes registers to scanner */
  regs[0x32] = 0x00;
  status = x1100_write_all_regs (devnum, regs);
  if (status != SANE_STATUS_GOOD)
    return status;
  regs[0x32] = 0x40;
  status = x1100_write_all_regs (devnum, regs);
  ...
  DBG (2, "x1100_start_scan: end.\n");
  return status;
}

/* wait for scan data being available */
static SANE_Status
x1100_poll_data (SANE_Int devnum)
{
  SANE_Status status;
  int loops = 0;
  size_t size;
  ...
  while (loops < 1000)
    {
      ...
      /* as stated in sanei_lexmark_x1100_search_home_bwd, we read
       * available data count twice */
      size = 4;
      status = x1100_usb_bulk_write (devnum, command4_block, &size);
      if (status != SANE_STATUS_GOOD)
	return status;
      size = 0x3;
      status = x1100_usb_bulk_read (devnum, result, &size);
      if (status != SANE_STATUS_GOOD)
	return status;
      size = 4;
	...
	return status;
      ...
      if (count != 0)
	{
	  DBG (15, "x1100_poll_data: %d bytes available\n", count);
	  return SANE_STATUS_GOOD;
	}
      loops++;
    }
 ...
 */
static SANE_Status
x1100_simple_scan (Lexmark_Device * dev, SANE_Byte * regs, int xoffset,
		   int pixels, int yoffset, int lines, SANE_Byte ** data)
{
  ...
  DBG (2, "x1100_simple_scan: start\n");
  DBG (15, "x1100_simple_scan: x=%d, pixels=%d (ex=%d), y=%d, lines=%d\n",
  ...
  *data = (SANE_Byte *) malloc (bpl * lines);
  if (*data == NULL)
    {
      DBG (2,
	   "x1100_simple_scan: failed to allocate %d bytes !\n", bpl * lines);
      return SANE_STATUS_NO_MEM;
    }

  /* start scan */
  status = x1100_cancel (dev->devnum);
  if (status != SANE_STATUS_GOOD)
    return status;


  status = x1100_start_scan (dev->devnum, regs);
  if (status != SANE_STATUS_GOOD)
    return status;

  /* wait for data */
  status = x1100_poll_data (dev->devnum);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (1, "x1100_simple_scan: time-out while waiting for data.\n");
      return status;
    }

  /* data reading loop */
  needed = bpl * lines;
  DBG (1, "x1100_simple_scan: bpl=%d, lines=%d, needed=%d.\n", bpl, lines,
       needed);
  read = 0;
  do
    {
  ...
  if (regs[0xc3] & 0x80)
      ...
      do
	{
	  if (rts88xx_read_reg (dev->devnum, 0xb3, &reg) != SANE_STATUS_GOOD)
	    {
	      DBG (5, "x1100_simple_scan: register read failed ...\n");
	      return SANE_STATUS_IO_ERROR;
	    }
	  usleep (100000);
	  i++;
	...
	{
	  DBG (5,
	       "x1100_simple_scan : timeout waiting for motor to stop ...\n");
	  return SANE_STATUS_IO_ERROR;
	}
    }
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (1, "x1100_simple_scan: cancel failed.\n");
      return status;
    }

  DBG (2, "x1100_simple_scan: end.\n");
  return status;
}
 ...
 */
SANE_Status
sanei_lexmark_x1100_open_device (Lexmark_Device * dev)
{
  ...
  size = 4;
  x1100_usb_bulk_write (dev->devnum, command_block, &size);
  size = 0xFF;
  x1100_usb_bulk_read (dev->devnum, shadow_regs, &size);
#ifdef DEEP_DEBUG
  if (DBG_LEVEL > 2)
    {
      fprintf (stderr,
	       "sanei_lexmark_x1100_open_device: initial registers values\n");
  ...
  if (variant != 0)
    {
      DBG (3,
	   "sanei_lexmark_x1100_open_device: reassign model/sensor for varaint 0x%02x\n",
	   variant);
      sanei_lexmark_x1100_assign_model (dev, dev->sane.name,
					dev->model.vendor_id,
					dev->model.product_id, variant);
      /* since model has changed, run init again */
      sanei_lexmark_x1100_init (dev);
    }
  DBG (2, "sanei_lexmark_x1100_open_device: end\n");
  return result;
}

void
sanei_lexmark_x1100_close_device (Lexmark_Device * dev)
{
  /* put scanner in idle state */
  lexmark_x1100_set_idle (dev->devnum);
...
/* This function writes the contents of the given registers to the 
     scanner. */
SANE_Status
x1100_write_all_regs (SANE_Int devnum, SANE_Byte * regs)
{
  int i;
  SANE_Status status;
  size_t size;
  ...
  status = x1100_usb_bulk_write (devnum, command_block1, &size);
  if (status != SANE_STATUS_GOOD)
    return status;
  size = 0x4f;
  status = x1100_usb_bulk_write (devnum, command_block2, &size);
  if (status != SANE_STATUS_GOOD)
    return status;
  return SANE_STATUS_GOOD;
}
			      ...
			      SANE_Byte * regs)
{
  ...
  DBG (2, "sanei_lexmark_x1100_move_fwd: \n");
  devnum = dev->devnum;
...
/* Move Forward without scanning: */
  regs[0x32] = 0x00;
  x1100_write_all_regs (devnum, regs);
  regs[0x32] = 0x40;
  x1100_write_all_regs (devnum, regs);
  ...
  DBG (2, "sanei_lexmark_x1100_move_fwd: end.\n");
}
...
sanei_lexmark_x1100_search_home_fwd (Lexmark_Device * dev)
{
  ...
  x1100_write_all_regs (devnum, dev->shadow_regs);
  dev->shadow_regs[0x32] = 0x40;
  x1100_write_all_regs (devnum, dev->shadow_regs);
  ...
  /* free the buffer */
  free (buffer);
  DBG (2, "sanei_lexmark_x1100_search_home_fwd: end.\n");
...
/* This function must only be called if the scan head is past the home dot.
  ...
  while (buffer_count < buffer_limit)
      ...
      while (!got_line)
	{
	  ...
	  cmd_size = 4;
	  status = x1100_usb_bulk_write (devnum, command4_block, &cmd_size);
	  if (status != SANE_STATUS_GOOD)
	    return SANE_FALSE;
	  cmd_size = 0x3;
	  status = x1100_usb_bulk_read (devnum, poll_result, &cmd_size);
	  if (status != SANE_STATUS_GOOD)
	    return SANE_FALSE;
	  cmd_size = 4;
	  status = x1100_usb_bulk_write (devnum, command4_block, &cmd_size);
	...
	no_of_buffers = 10;
      ...
      cmd_size = 4;
      status = x1100_usb_bulk_write (devnum, command5_block, &cmd_size);
      if (status != SANE_STATUS_GOOD)
	return SANE_FALSE;
      /* Read it */
      cmd_size = size_requested;
      status = x1100_usb_bulk_read (devnum, buffer, &cmd_size);
      if (status != SANE_STATUS_GOOD)
	return SANE_FALSE;
      for (i = 0; i < no_of_buffers; i++)
	{
	  ...
	  if (in_home_region)
		...
		sleep (1);
	      ...
	      i = ftell (img) / 2500;
	      rewind (img);
	      DBG (2, "sanei_lexmark_x1100_search_home_bwd: offset=%d\n", i);
	      fprintf (img, "P5\n2500 %03d\n", i);
	      fclose (img);
#endif
	      x1100_stop_mvmt (devnum);
	      DBG (2,
		   "sanei_lexmark_x1100_search_home_bwd: in home region, end.\n");
	      return SANE_TRUE;
	    }
	}
  ...
  DBG (2, "sanei_lexmark_x1100_search_home_bwd: end.\n");
...
void
x1100_set_scan_area (SANE_Int res,
		     SANE_Int tlx,
		     SANE_Int tly,
		     SANE_Int brx,
		     SANE_Int bry,
		     ...
		     SANE_Byte * regs, Lexmark_Device * dev)
{
  ...
  status = x1100_get_start_loc (res, &vert_start, &hor_start, offset, dev);
  ...
  /* Debug */
  DBG (2, "x1100_set_scan_area: vert_start: %d (tly=%d)\n", vert_start, tly);
  DBG (2, "x1100_set_scan_area: vert_end: %d\n", vert_end);
  DBG (2, "x1100_set_scan_area: hor_start: %d\n", hor_start);
  DBG (2, "x1100_set_scan_area: hor_end: %d\n", hor_end);

}

SANE_Int
sanei_lexmark_x1100_find_start_line (Lexmark_Device * dev)
{
  /*
  ...
  DBG (2, "sanei_lexmark_x1100_find_start_line:\n");
  ...
  /* Stop the scanner */
  x1100_stop_mvmt (dev->devnum);

  /* write regs out twice */
  dev->shadow_regs[0x32] = 0x00;
  x1100_write_all_regs (dev->devnum, dev->shadow_regs);
  dev->shadow_regs[0x32] = 0x40;
  x1100_write_all_regs (dev->devnum, dev->shadow_regs);
  ...
  while (!got_line)
    {
      cmd_size = 4;
      x1100_usb_bulk_write (dev->devnum, command4_block, &cmd_size);
      cmd_size = 0x3;
      x1100_usb_bulk_read (dev->devnum, poll_result, &cmd_size);
      if (!
	  (poll_result[0] == 0 && poll_result[1] == 0 && poll_result[2] == 0))
	{
	  /* if result != 00 00 00 we got data */
  ...
  /* Write: 91 00 14 48 */
  cmd_size = 4;
  x1100_usb_bulk_write (dev->devnum, command5_block, &cmd_size);
  /* Read it */
  cmd_size = 0x1448;
  x1100_usb_bulk_read (dev->devnum, buffer, &cmd_size);
  ...
  DBG (2, "sanei_lexmark_x1100_find_start_line: end.\n");
				   ...
				   SANE_Bool calibrated)
{
  ...
  DBG (2, "sanei_lexmark_x1100_set_scan_regs:\n");
  /* resolution */
  yres = dev->val[OPT_RESOLUTION].w;

  DBG (7, "sanei_lexmark_x1100_set_scan_regs: yres=%d DPI\n", yres);
  ...
  DBG (2, "sanei_lexmark_x1100_set_scan_regs: end.\n");
  return SANE_STATUS_GOOD;
}
...
sanei_lexmark_x1100_start_scan (Lexmark_Device * dev)
{
  ...
  DBG (2, "sanei_lexmark_x1100_start_scan: end.\n");
  return SANE_STATUS_GOOD;
}

long
sanei_lexmark_x1100_read_scan_data (SANE_Byte * data, SANE_Int size,
				    Lexmark_Device * dev)
{
  ...
  if (dev->transfer_buffer == NULL)
      ...
      if (dev->bytes_remaining > 0)
	{
	  ...
	  command1_block[3] = (SANE_Byte) (xfer_request & 0xFF);
          ...
          if (status != SANE_STATUS_GOOD)
            {
              DBG (1, "sanei_lexmark_x1100_read_scan_data: time-out while waiting for data.\n");
              return status;
            }
	  ...
	  cmd_size = 4;
	  x1100_usb_bulk_write (dev->devnum, command1_block, &cmd_size);

	  /* Read: xfer_size  bytes */
	  cmd_size = xfer_request;
	  x1100_usb_bulk_read (dev->devnum, dev->transfer_buffer, &cmd_size);
  ...
  if ((dev->bytes_remaining == 0) && read_buffer_is_empty (dev->read_buffer))
	...
	{
	  DBG (2,
	       "sanei_lexmark_x1100_read_scan_data: EOF- parking the scanner\n");
	  dev->eof = SANE_TRUE;
	  x1100_rewind (dev, dev->shadow_regs);
	}
      else
	{
  ...
  DBG (2, "sanei_lexmark_x1100_read_scan_data: end.\n");
  return bytes_read;
}

void
x1100_rewind (Lexmark_Device * dev, SANE_Byte * regs)
{
  ...
  DBG (2, "x1100_rewind: %d=>new_location=%d\n", dev->val[OPT_BR_Y].w,
       new_location);

  /* stops any pending scan */
  x1100_clr_c6 (dev->devnum);
  x1100_cancel (dev->devnum);
...
/* set regs for rewind */
  ...
  /* starts scan */
  x1100_start_scan (dev->devnum, regs);
  DBG (2, "x1100_rewind: end.\n");
}
 ...
 */
SANE_Status
sanei_lexmark_x1100_offset_calibration (Lexmark_Device * dev)
{
  ...
  DBG (2, "sanei_lexmark_x1100_offset_calibration: start\n");
  /* copy registers */
  for (i = 0; i < 255; i++)
  ...
  /* minimal gains */
  DBG (3,
       "sanei_lexmark_x1100_offset_calibration: setting gains to (1,1,1).\n");
  ...
  while (((i > 0) && (average > dev->sensor->offset_threshold))
	 || (i == OFFSET_RANGES))
    {
      ...
      DBG (3,
	   "sanei_lexmark_x1100_offset_calibration: setting offsets to (%d,%d,%d).\n",
	   ro, ro, ro);

      status =
	x1100_simple_scan (dev, regs, dev->sensor->offset_startx, pixels,
			   yoffset, lines, &data);
      if (status != SANE_STATUS_GOOD)
	{
	  DBG (1,
	       "sanei_lexmark_x1100_offset_calibration: x1100_simple_scan failed!\n");
	  if (data != NULL)
	    free (data);
	  return status;
	}
  ...
  if (i == 0)
    {
      DBG (2, "sanei_lexmark_x1100_offset_calibration: failed !\n");
      failed=1;
    }
  ...
  status =
    x1100_simple_scan (dev, regs, dev->sensor->offset_startx, pixels, yoffset,
		       lines, &data);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (1,
  ...
  DBG (7,
       "sanei_lexmark_x1100_offset_calibration: offset=(0x%02x,0x%02x,0x%02x).\n",
       dev->offset.red, dev->offset.green, dev->offset.blue);

  DBG (2, "sanei_lexmark_x1100_offset_calibration: end.\n");
  free (data);
  return status;
}
 ...
 */
SANE_Status
sanei_lexmark_x1100_gain_calibration (Lexmark_Device * dev)
{
  SANE_Byte regs[255];		/* we have our own copy of shadow registers */
  SANE_Status status = SANE_STATUS_GOOD;
  ...
  DBG (2, "sanei_lexmark_x1100_gain_calibration: start\n");
  /* copy registers */
  for (i = 0; i < 255; i++)
    regs[i] = dev->shadow_regs[i];
  ...
  while (((rts88xx_is_color (regs)
    ...
    {
      status =
	x1100_simple_scan (dev, regs, sx, pixels, yoffset, lines, &data);
      if (status != SANE_STATUS_GOOD)
	{
	  DBG (1,
	       "sanei_lexmark_x1100_gain_calibration: x1100_simple_scan failed!\n");
	  if (data != NULL)
	    free (data);
	  return status;
	}
  ...
  dev->gain.gray = green;
  DBG (7,
       "sanei_lexmark_x1100_gain_calibration: gain=(0x%02x,0x%02x,0x%02x).\n",
       dev->gain.red, dev->gain.green, dev->gain.blue);
  DBG (2, "sanei_lexmark_x1100_gain_calibration: end.\n");
  return status;
}
 ...
 */
SANE_Status
sanei_lexmark_x1100_shading_calibration (Lexmark_Device * dev)
{
  SANE_Byte regs[255];		/* we have our own copy of shadow registers */
  int i, j, pixels, bpl;
  int sx, ex;
  ...
  DBG (2, "sanei_lexmark_x1100_shading_calibration: start\n");
  /* copy registers */
  for (i = 0; i < 255; i++)
    regs[i] = dev->shadow_regs[i];
  ...
  if (data == NULL)
    {
      DBG (2,
	   "sanei_lexmark_x1100_shading_calibration: failed to allocate %d bytes !\n",
	   bpl * lines);
      return SANE_STATUS_NO_MEM;
    }
  ...
  if (dev->shading_coeff == NULL)
    {
      DBG (2,
	   "sanei_lexmark_x1100_shading_calibration: failed to allocate %d floats !\n",
	   bpl);
      free (data);
      return SANE_STATUS_NO_MEM;
    }
  ...
  /* execute scan */
  status = x1100_simple_scan (dev, regs, sx, pixels, yoffset, lines, &data);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (1,
	   "sanei_lexmark_x1100_shading_calibration: x1100_simple_scan failed!\n");
      if (data != NULL)
	free (data);
      return status;
    }
  ...
  if (yoffset >= lines - 1)
    {
      DBG (7,
	   "sanei_lexmark_x1100_shading_calibration: failed to detect yoffset.\n");
    ...
    yoffset++;
  DBG (7, "sanei_lexmark_x1100_shading_calibration: yoffset=%d.\n", yoffset);
  ...
  /* execute scan */
  status = x1100_simple_scan (dev, regs, sx, pixels, 1, lines, &data);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (1,
	   "sanei_lexmark_x1100_shading_calibration: x1100_simple_scan failed!\n");
  ...
  DBG (2, "sanei_lexmark_x1100_shading_calibration: end.\n");
  return status;
}


SANE_Status
sanei_lexmark_x1100_calibration (Lexmark_Device * dev)
{
  SANE_Status status;

  status = sanei_lexmark_x1100_offset_calibration (dev);
  if (status != SANE_STATUS_GOOD)
    return status;
  ...
  if (dev->val[OPT_MANUAL_GAIN].w == SANE_TRUE)
    ...
    {
      status = sanei_lexmark_x1100_gain_calibration (dev);
      if (status != SANE_STATUS_GOOD)
	return status;
    }
  ...
  status = sanei_lexmark_x1100_shading_calibration (dev);
  if (status != SANE_STATUS_GOOD)
    return status;

  DBG (2, "sanei_lexmark_x1100_calibration: end.\n");
  return SANE_STATUS_GOOD;
}

/* assign sensor data */
static SANE_Status
sanei_lexmark_x1100_assign_sensor (Lexmark_Device * dev)
{
  int dn;
  ...
  if (sensor_list[dn].id == 0)
    {
      DBG (1,
	   "sanei_lexmark_x1100_assign_sensor: unknown sensor %d\n",
	   dev->model.sensor_type);
      return SANE_STATUS_UNSUPPORTED;
    }
  dev->sensor = &(sensor_list[dn]);
  DBG (1, "sanei_lexmark_x1100_assign_sensor: assigned sensor number %d\n",
       dev->model.sensor_type);
  return SANE_STATUS_GOOD;
}
 ...
 * available */
SANE_Status
sanei_lexmark_x1100_assign_model (Lexmark_Device * dev, char *devname,
				  ...
				  SANE_Byte mainboard)
{
  ...
  DBG (2, "sanei_lexmark_x1100_assign_model: start\n");
  DBG (3,
       "sanei_lexmark_x1100_assign_model: assigning %04x:%04x, variant %d\n",
  ...
  if (!found)
    {
      DBG (1,
	   "sanei_lexmark_x1100_assign_model: unknown device 0x%04x:0x%04x\n",
	   vendor, product);
      return SANE_STATUS_UNSUPPORTED;
    }
  ...
  DBG (3, "sanei_lexmark_x1100_assign_model: assigned %s\n",
       dev->model.model);

  /* init sensor data */
  return sanei_lexmark_x1100_assign_sensor (dev);
  DBG (2, "sanei_lexmark_x1100_assign_model: end.\n");
}
