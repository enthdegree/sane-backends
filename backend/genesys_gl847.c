...
static SANE_Status
gl847_bulk_write_data (Genesys_Device * dev, uint8_t addr,
		       uint8_t * data, size_t len)
{
  ...
  DBG (DBG_io, "gl847_bulk_write_data writing %lu bytes\n", (u_long) len);
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "gl847_bulk_write_data failed while setting register: %s\n",
  ...
  while (len)
	...
	{
	  DBG (DBG_error,
	       "gl847_bulk_write_data failed while writing command: %s\n",
	       sane_strstatus (status));
	  return status;
	}
	...
	{
	  DBG (DBG_error,
	       "gl847_bulk_write_data failed while writing bulk data: %s\n",
	       sane_strstatus (status));
	  return status;
	}

      DBG (DBG_io2,
	   "gl847_bulk_write_data wrote %lu bytes, %lu remaining\n",
  ...
  DBG (DBG_io, "gl847_bulk_write_data: completed\n");
 ...
 * first read is rounded to a multiple of 512 bytes, last raed fetches the 
 * remainder
 */
static SANE_Status
		      ...
		      uint8_t * data, size_t len)
{
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "gl847_bulk_read_data failed while setting register: %s\n",
	   sane_strstatus (status));
      return status;
    }
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "gl847_bulk_read_data failed while writing command: %s\n",
	   sane_strstatus (status));
      return status;
    }
  ...
  while (len)
      ...
      if (status != SANE_STATUS_GOOD)
	{
      ...
      DBG (DBG_io2,
	   "gl847_bulk_read_data read %lu bytes, %lu remaining\n",
	   (u_long) size, (u_long) (len - size));
  ...
  DBG (DBG_io, "gl847_bulk_read_data: completed\n");
			...
			uint16_t * slope_table, int steps)
{
  ...
  free (table);
  DBG (DBG_proc, "gl847_send_slope_table: completed\n");
  return status;
}
...
gl847_set_fe (Genesys_Device * dev, uint8_t set)
{
  ...
  DBG (DBG_proc, "gl847_set_fe: completed\n");
...
static SANE_Status
gl847_init_motor_regs_off (Genesys_Device * dev, Genesys_Register_Set * reg, unsigned int scan_lines	/*lines, scan resolution */
  )
{
  unsigned int feedl;
  Genesys_Register_Set *r;
 ...
 */
  ...
  DBG (DBG_proc, "gl847_init_motor_regs_off : completed. \n");
			    ...
			    int scan_power_mode, unsigned int flags)
{
  ...
  gl847_send_slope_table (dev, 0, slow_slope_table, 256);
  gl847_send_slope_table (dev, 1, slow_slope_table, 256);
  gl847_send_slope_table (dev, 2, slow_slope_table, 256);
  gl847_send_slope_table (dev, 3, slow_slope_table, 256);
  gl847_send_slope_table (dev, 4, slow_slope_table, 256);
...
/* END TODO */
  ...
  DBG (DBG_proc, "gl847_init_scan_regs: completed\n");
  return SANE_STATUS_GOOD;
}
...
/* exposure_time */
  ...
  DBG (DBG_info, "gl847_calculate_current_setup : exposure_time=%d pixels\n",
       exposure_time);
...
/* lincnt */
  ...
  DBG (DBG_proc, "gl847_calculate_current_setup: completed\n");
  return SANE_STATUS_GOOD;
}
		  ...
		  SANE_Bool start_motor)
{
  ...
  status =
    gl847_bulk_write_register (dev, local_reg,
			       sizeof (local_reg) / sizeof (local_reg[0]));
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "gl847_begin_scan: Failed to bulk write registers: %s\n",
	   sane_strstatus (status));
      return status;
    }
		...
		SANE_Bool check_stop)
{
  ...
  DBG (DBG_proc, "gl847_end_scan: completed\n");
...
gl847_init_regs_for_coarse_calibration (Genesys_Device * dev)
{
  ...
  DBG (DBG_proc, "gl847_init_register_for_coarse_calibration: completed\n");

/*  if (DBG_LEVEL >= DBG_info)
    sanei_gl847_print_registers (dev->calib_reg);*/
...
/*				 SCAN_FLAG_DISABLE_BUFFER_FULL_MOVE |*/
  ...
  DBG (DBG_proc, "gl847_init_regs_for_shading: completed\n");
...
/* start */
  ...
  DBG (DBG_proc, "gl847_init_register_for_scan: completed\n");
  return SANE_STATUS_GOOD;
}
...
gl847_led_calibration (Genesys_Device * dev)
{
  ...
  if (dev->model->gpo_type == GPO_CANONLIDE200)
    {
      status = gl847_feed (dev, 280);	/*feed to white strip. canon lide 35 only. */
...
gl847_offset_calibration (Genesys_Device * dev)
{
  ...
  DBG (DBG_proc, "gl847_offset_calibration\n");
 ...
 */
  ...
  do
      ...
      if (status != SANE_STATUS_GOOD)
	{
      ...
      DBG (DBG_info,
	   "gl847_offset_calibration: starting first line reading\n");
      RIE (gl847_begin_scan (dev, dev->calib_reg, SANE_TRUE));
      ...
      for (j = 0; j < channels; j++)
	{
      ...
      DBG (DBG_info, "gl847_offset_calibration: black/white pixels: "
	   "%d/%d,%d/%d,%d/%d\n",
  ...
  DBG (DBG_info, "gl847_offset_calibration: acceptable offsets: %d,%d,%d\n",
       off[0], off[1], off[2]);
  ...
  DBG (DBG_info, "gl847_offset_calibration: first set: %d/%d,%d/%d,%d/%d\n",
       off1[0], min1[0], off1[1], min1[1], off1[2], min1[2]);

  DBG (DBG_info, "gl847_offset_calibration: second set: %d/%d,%d/%d,%d/%d\n",
...
/*TODO: try to avoid this*/
	  DBG (DBG_warn, "gl847_offset_calibration: difference too small\n");
	...
	off[j] =
	  (mintgt * (off1[j] - off2[j]) + min1[j] * off2[j] -
	   min2[j] * off1[j]) / (min1[j] - min2[j]);
      if (off[j] > 255)
	off[j] = 255;
      if (off[j] < 0)
	off[j] = 0;
  ...
  DBG (DBG_info, "gl847_offset_calibration: final offsets: %d,%d,%d\n",
       off[0], off[1], off[2]);
  ...
  free (first_line);
  free (second_line);
  DBG (DBG_proc, "gl847_offset_calibration: completed\n");
  return status;
}
...
gl847_coarse_gain_calibration (Genesys_Device * dev, int dpi)
{
  ...
  DBG (DBG_proc, "gl847_coarse_gain_calibration\n");
  ...
  if (dev->model->gpo_type == GPO_CANONLIDE200)
	...
	{
	  DBG (DBG_error,
	       "gl847_coarse_gain_calibration: Failed to feed: %s\n",
	       sane_strstatus (status));
	  return status;
	}
    }
  ...
  for (j = 0; j < channels; j++)
      ...
      for (i = 0; i < num_pixels; i++)
	{
      ...
      if (dev->model->dac_type == DAC_CANONLIDE200 ||
  ...
  DBG (DBG_proc, "gl847_coarse_gain_calibration: completed\n");
  return status;
}
...
/* okay.. these should be defaults stored somewhere */
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "gl847_init_regs_for_warmup: Failed to setup scan: %s\n",
	   sane_strstatus (status));
      return status;
    }
...
gl847_init (Genesys_Device * dev)
{
  ...
  sanei_genesys_write_register (dev, REG_GPIO17_21, val | 0x04);
  sanei_genesys_write_register (dev, REG_GPOE17_21, 0x0f);
...
gl847_update_hardware_sensors (Genesys_Scanner * s)
{
  ...
  if (s->dev->model->gpo_type == GPO_CANONLIDE200)
    {
...
gl847_search_strip (Genesys_Device * dev, SANE_Bool forward, SANE_Bool black)
{
  ...
  DBG (DBG_proc, "gl847_search_strip: completed\n");
  return status;
}
